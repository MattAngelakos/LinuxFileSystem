...
{
	//Gets the image from the block device and return its number of blocks
...
{
    struct image_dev *im = dev->private;
    if (first_blk == 0){ //If first block is equal to 0, prints error
        fprintf(stderr, "write error on %s: %s\n", im->path, strerror(errno));
    }
    if (im->fd == -1) { //If file descriptor is equal -1, returns that it is unavailable
        return E_UNAVAIL;
    }
    assert(first_blk >= 0 && first_blk+nblks <= im->nblks);
     ...
     */
    if (result < 0) {
        fprintf(stderr, "read write on %s: %s\n", im->path, strerror(errno));
        assert(0);
    }
    if (result != nblks*BLOCK_SIZE) {
...
{
	//Checks if the file descriptor is equal to -1, as long as it isn't, it should be able to flush and therefore returns Success
    struct image_dev *im = dev->private;
    if (im->fd == -1) {
        return E_UNAVAIL;
    }
...
{
	//Closes the file descriptor and frees the image and the block device
	struct image_dev *im = dev->private;
	if (im->fd != -1) {
		close(im->fd);
	}
