...
int fs_utime(const char *path, struct utimbuf *ut)
{
    ...
    if (inode_idx < 0) return inode_idx;
    struct fs_inode *inode = &inodes[inode_idx];
    inode->mtime = ut->modtime; //Replaces the node's current mtime with the modification time of utime
    update_inode(inode_idx); //Updates the node with the modified node
    return SUCCESS;
}
...
static void fs_read_blk(int blk_num, char *buf, size_t len, size_t offset) {
	...
	memcpy(buf, entries + offset, len); //Data from the entries array of input length is copied to the buf
}
